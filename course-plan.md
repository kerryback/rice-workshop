Faculty Workshop: Claude for Teaching and Research
3:00 - 4:00 PM
https://jgsb.zoom.us/j/95657980099?pwd=Ve5TMEaaBRQnhWnhnpaJFDHevFIbRU.1


**AUG 10:** **Intro to Claude.** Claude Desktop (Chat, Cowork, Code), Claude plugin for Excel

**AUG 11:** **Claude Tools.** Code execution, command line, web search and fetch, and browser use

**AUG 12:** **Skills, Connectors, and Plugins.** Customizing Claude and adding capabilities

**AUG 13:** **AI for Teaching.** NotebookLM, RAG chatbots, slide voice-overs

**AUG 14:** **Student Projects.** Artifacts, apps, agents

**AUG 17:** **Research Setup.** Academic Studio, Python, R, LaTeX, Quarto, Git, GitHub

**AUG 18:** **AI for Research** Literature search, knowledge databases, writing, and reviews

**AUG 19** **Agent Assistants** Research team, personal assistants
